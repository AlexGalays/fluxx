...
exports['default'] = (function () {
  ...
  function dispatch(action, payloads) {
    ...
    if (dispatcher.log) {
      var msg = logVerbosity() > 0 ? tryStringify(payloads) : payloads;
  ...
  function notifyStore(id) {
    ...
    if (dispatcher.log && result !== undefined) {
      var updateMsg = result === false ? '(did not update its UI state)' : store._type == 'Store' ? 'New state: ' : '';

      var newState = logVerbosity() == 2 ? tryStringify(store.state) : store.state;
